...
| **cobertura por PACOTE, não só a média** | `coverage-gate.sh` | avisa por padrão, reprova com `STRICT_PACKAGES=1`. Hoje 8 pacotes abaixo de 90% — a média de 90,2% os escondia |
| **o aviso sai da máquina e chega ao destino** | `41-setup-notify` | 6 seções: dono e modo do arquivo, o modelo não o lê, entrega real ao ntfy, drenagem pela unidade |
